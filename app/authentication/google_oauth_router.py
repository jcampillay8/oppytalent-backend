...
async def google_auth_callback(
    ...
    # --- Respuesta y Cookies ---
    from app.authentication.router import get_cookie_settings # Import local para evitar circular
    
    base_url = str(settings.WEBSITE_URL).rstrip('/')
    response = RedirectResponse(url=f"{base_url}/auth/callback?token={access_token}", status_code=status.HTTP_302_FOUND)
